									...
									<div class="table-responsive">
										<table class="table table-hover"
											...
											<tbody>
												...
												<template v-for="(dpostscript, i) in currentList">
													<dpostscript :key="dpostscript.dpostscript_idx" :num="num-i"
														:obj="dpostscript" />
												</template>
												<!-- 레코드 한 줄 -->
											</tbody>
...
const dpostscript={
	template:`
        <tr>
            <td>{{n}}</td>
            <td v-on:click="getDetail(json.dpostscript_idx)">{{json.dpostscript_title}}</td>
            <td>{{(json.dpostscript_date).substring(0, 10)}}</td>
    	</tr>
	`,
	...
	methods:{
		getDetail(idx){
			location.href="/postscript/detail?dpostscript_idx="+idx
		}
	}
};
app1=new Vue({
...
function pageLink(n){//원하는 페이지만 넘겨받기
	...
	pager.init(app1.dpostscriptList, n);
	...
	for(let i=pager.curPos; i<pager.curPos+pager.pageSize;i++){	
		...
		//전체 배열에서, 일부 배열로 옮겨심기
		app1.currentList.push(app1.dpostscriptList[i]);
